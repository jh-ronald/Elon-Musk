...
t_1722 <- bind_rows(elon_tweets_1718 %>%
                      mutate(person = "EM1718"),
                      elon_tweets_2122 %>%
                      mutate(person = "EM2122"))

ggplot(t_1722, aes(x = Date, fill = person)) +
geom_histogram(position = "identity", bins = 20, show.legend = FALSE) +
facet_wrap(~person, ncol = 1)

remove_reg <- "&amp;|&lt;|&gt;"
tidy_tweets_1722 <- t_1722 %>%
...
frequency <- tidy_tweets_1722 %>%
count(person, word, sort = TRUE) %>%
left_join(tidy_tweets_1722 %>%
...
frequency <- frequency %>%
  ...
  arrange(EM1718, EM2122)

ggplot(frequency, aes(EM1718, EM2122)) +
